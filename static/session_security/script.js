...
yourlabs.SessionSecurity = function(options) {
    ...
    // Commented to enable the custom onbefore unload on main javascript
...
yourlabs.SessionSecurity.prototype = {
    ...
    apply: function() {
        ...
        if (idleFor >= this.expireAfter) {
            ...
            x = $("#timeout-count").text(nextPing);
            setInterval(function(){ 
                x = $("#timeout-count").text();
                y = parseInt(x) - 1;
                $("#timeout-count").text(y); 
            }, 1000);
        } else {
            this.hideWarning();
